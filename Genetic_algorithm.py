...
class Individual:
    ...
    def calc_fitness(self):
        fitness = sum(
            [self.graph.calculate_distance(self.path[i], self.path[i + 1]) for i in range(len(self.path) - 1)])
        return fitness
...
class Generation:
    ...
    def get_child(self):
        ...
        parent1 = ind1 if ind1.fitness < ind2.fitness else ind2

        ind1 = choice(self.population)
        ind2 = choice(self.population)
        parent2 = ind1 if ind1.fitness < ind2.fitness else ind2
...
class GeneticAlgoritm(Algorithm):
    def __init__(self, graph, population_size=100, generations=300, mutation=0.03):
...
if __name__ == "__main__":
    ...
    fig.add_scatter(x=list(range(1, len(best_inds) + 1)), y=best_inds, mode="markers+lines", name="Best_paths")
