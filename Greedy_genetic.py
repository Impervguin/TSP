...
from Genetic_algorithm import GeneticAlgoritm, Individual, Generation
import plotly as px

class GreedyIndividual(Individual):
    def get_child(self, other):
        ...
        for i in range(len(child_perm)):
            ...
            if r < self.mutation_chance:
                ...
                m = (-1,  100000000)
                for j in range(len(child_perm)):
                    if i == j or j == self.path[0]:
                        continue
                    d = self.graph.get_distance(i, j)
                    if d < m[1]:
                        m = (j, d)
                child_perm[i], child_perm[m[0]] = child_perm[m[0]], child_perm[i]
                break
        return GreedyIndividual(self.path[0], self.graph, child_perm, self.mutation_chance)

class GreedyGenetic(GeneticAlgoritm):
    def solve(self, start_point=0):
        ...
        for _ in range(self.population_size):
            ...
            shuffle(perm)
            population.append(GreedyIndividual(start_point, self.graph, perm, self.mutation))
...
if __name__ == "__main__":
    ...
    # graph = Graph(*points)
    # algorithm = GeneticAlgoritm(graph, mutation=0.5)
    # start = 1 # 9, 1
