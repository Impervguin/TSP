...
class SpecialAntAlgorithm(Algorithm):
    ...
    def solve(self, start=0, ants=30, iter=1500, anti_ants=10):
        ...
        for i in range(iter):
            ...
            for a_a in range(anti_ants):
                ...
                path.append(start)
                dist += self.matrix[path[-2]][path[-1]][0]
                anti_ants_paths.append((path, dist))
                if dist < min_dist:
                    min_path, min_dist = path, dist
            ...
            for a_ap in anti_ants_paths:
                pheromones = PHEROMONES_ENCH / a_ap[1]
                for j in range(len(ap[0]) - 1):
                    self.matrix[ap[0][j]][ap[0][j + 1]][1] -= pheromones
                    if self.matrix[ap[0][j]][ap[0][j + 1]][1] < 0:
                        self.matrix[ap[0][j]][ap[0][j + 1]][1] = 0
            # if i % 10 == 9:
...
if __name__ == "__main__":
    ...
    graph = Graph(*points)
    algorithm = SpecialAntAlgorithm(graph)
    start = 1  # 9, 1
